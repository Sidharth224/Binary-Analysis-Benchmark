...
export OUT_DIR=/home/sidharth/Binary-dataset-CVE-beta/unit-testing-benchmark/RTOS-specific-binaries/Contiki-ng
...
if [ -d $OUT_DIR/contiki-$CONTIKI_VER ]; then
    rm -r $OUT_DIR/contiki-$CONTIKI_VER
fi
mkdir $OUT_DIR/contiki-$CONTIKI_VER
    ...
    for BOARD in $BOARDS; do
        if [ -d $CONTIKI_EXAMPLE_DIR/$EXAMPLE ]; then
            ...
            if [ -d $CONTIKI_EXAMPLE_DIR/$EXAMPLE/build ]; then
                ...
                if [ ! -d $OUT_DIR/contiki-$CONTIKI_VER/$EXAMPLE_NAME ]; then
                    mkdir $OUT_DIR/contiki-$CONTIKI_VER/$EXAMPLE_NAME
                fi
                ...
                rm -rv *
                unset GLOBIGNORE
                cp -r $BUILD_DIR_TEMP $OUT_DIR/contiki-$CONTIKI_VER/$EXAMPLE_NAME/
                #if [ -f $CONTIKI_EXAMPLE_DIR/$EXAMPLE/$EXAMPLE_NAME.elf]; then
                #    find *.bin -type f -exec cp "{}"  $OUT_DIR/contiki-$CONTIKI_VER/$EXAMPLE_NAME/ \;
                #fi 
                
                #if [ -f $CONTIKI_EXAMPLE_DIR/$EXAMPLE/$EXAMPLE_NAME.bin]; then
                #    find *.elf -type f -exec cp "{}"  $OUT_DIR/contiki-$CONTIKI_VER/$EXAMPLE_NAME/ \;
                #fi    
            fi
